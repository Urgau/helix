...
use helix_core::{SmallVec, Tendril};
...
fn render_helper<'a>(
    snippet_elements: &Vec<SnippetElement<'a>>,
    insert: &mut String,
    offset: &mut usize,
        ...
        match element {
            ...
            Placeholder { tabstop, value: inner_snippet_elements } =>  {
                ...
                if include_placeholer {
                    render_helper(&inner_snippet_elements, insert, offset, tabstops, newline_with_offset, include_placeholer);
                }
...
pub fn render<'a>(
    ...
    newline_with_offset: String,
    include_placeholer: bool,
) -> (Option<Tendril>, Vec<SmallVec<[(usize, usize);1]>>) {
    let mut insert = String::new();
    let mut offset = 0;
    let mut tabstops: Vec<(usize, (usize, usize))> = Vec::new();
    render_helper(&snippet.elements, &mut insert, &mut offset, &mut tabstops, &newline_with_offset, include_placeholer);
    ...
    let mut ntabstops = Vec::<SmallVec<[(usize, usize); 1]>>::new();
    {
        ...
        for (tabstop, r) in tabstops {
            if prev == Some(tabstop) {
                ...
                prev = Some(tabstop);
                ntabstops.push(helix_core::smallvec![r]);
            }
        }
    }
